...
const fetchSymbolData = async (query) => {
  const API = (import.meta.env.VITE_REACT_APP_IEX_API_PUBLIC_KEY)
  try {
    const response = await fetch(
      `https://cloud.iexapis.com/stable/stock/${query}/batch?types=quote,company,news,intraday-prices&last=1&token=${API}`
    );
